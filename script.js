...
function createProductCard(item, index = 0) {
    ...
    const alergenosBadges = (item.alergenos && item.alergenos.length)
        ? `<div class="mb-2">${item.alergenos.map(a => `<span class="allergen-badge"><i class="fa-solid fa-triangle-exclamation"></i> ${escapeHtml(a)}</span>`).join('')}</div>`
        : '';

    const priceHtml = item.originalPrice
        ? `<span class="card-price-orig">${formatBRL(item.originalPrice)}</span><span class="price-vibrant fw-black" style="font-size: 1.4rem;">${formatBRL(item.price)}</span>`
        : `<span class="price-vibrant fw-black" style="font-size: 1.4rem;">${formatBRL(item.price)}</span>`;

    const btnHtml = soldOut
        ? `<button class="btn-add-cart disabled" disabled aria-label="Esgotado"><i class="fa-solid fa-ban"></i></button>`
        : `<button class="btn-add-cart" onclick="addToCart('${escapeAttr(item.id)}', '${escapeAttr(item.name)}', ${item.price}, this)" aria-label="Adicionar ao carrinho"><i class="fas fa-plus"></i></button>`;
    ...
    return `
        <div class="col-12 col-md-6 col-lg-4 menu-item-col" data-category="${escapeHtml(item.category)}" data-name="${escapeHtml(item.name.toLowerCase())}" data-id="${escapeAttr(item.id)}">
            <div class="card h-100 rounded-5 overflow-hidden product-card${soldOut ? ' product-card-soldout' : ''}" style="animation: slideUp 0.5s ease forwards; animation-delay: ${index * 0.05}s">
...
// ── Conteúdo de congelados (FG_CONFIG) ──
function renderInfoSections() {
    const cfg = (window.FG_CONFIG || {}).congelados || {};
    const preparoList = document.getElementById('preparoList');
    if (preparoList && Array.isArray(cfg.preparo) && cfg.preparo.length) {
        preparoList.innerHTML = cfg.preparo.map(p =>
            `<li><strong>${escapeHtml(p.titulo)}:</strong> ${escapeHtml(p.texto)}</li>`
        ).join('');
    }
    const entregaText = document.getElementById('entregaText');
    if (entregaText && cfg.entrega) entregaText.textContent = cfg.entrega;
}

// ── Status da loja (opcional) ──
function checkStoreStatus() {
    if (!(window.FG_CONFIG && window.FG_CONFIG.showStoreStatus)) return;
...
window.checkout = async function () {
    ...
    const minOrder = window.FG_CONFIG && Number(window.FG_CONFIG.minOrder) > 0 ? Number(window.FG_CONFIG.minOrder) : 0;
    if (minOrder > 0) {
        const subtotal = cart.reduce((t, i) => t + (i.price * i.quantity), 0);
        if (subtotal < minOrder) {
            showToast(`Pedido mínimo de ${formatBRL(minOrder)}. Subtotal atual: ${formatBRL(subtotal)}.`);
            return;
        }
    }
